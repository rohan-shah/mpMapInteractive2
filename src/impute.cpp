...
#include <map>
#include <math.h>
template<bool hasLOD, bool hasLKHD> bool imputeInternal(unsigned char* theta, std::vector<double>& levels, double* lod, double* lkhd, int nMarkers, std::vector<int>& groups, int group, std::string& error)
{
...
bool impute(unsigned char* theta, std::vector<double>& thetaLevels, double* lod, double* lkhd, int nMarkers, std::vector<int>& groups, int group, std::string& error)
{
	...
	{
		return imputeInternal<true, true>(theta, thetaLevels, lod, lkhd, nMarkers, groups, group, error);
	}
	else if(lod != NULL && lkhd == NULL)
	{
		return imputeInternal<true, false>(theta, thetaLevels, lod, lkhd, nMarkers, groups, group, error);
	}
	else if(lod == NULL && lkhd != NULL)
	{
		return imputeInternal<false, true>(theta, thetaLevels, lod, lkhd, nMarkers, groups, group, error);
	}
	else
	{
		return imputeInternal<false, false>(theta, thetaLevels, lod, lkhd, nMarkers, groups, group, error);
	}
}
